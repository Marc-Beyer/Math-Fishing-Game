/**
 * Handle the basic game-loop
 * and all gameObjects and the camera
 */
class GameManager{
    
    // The last time the game-loop was called
    // used for calculating the fps and delta-time
    lastTimeStamp = 0;

    // All current used gameObjects
    gameObjectContainer = [];

    // Track if the game-loop is running
    _isActive = true;
    ...
    // starts game-loop if set to true
    get isActive() {
        return this._isActive;
    }
    set isActive(boolean) {
        if(this._isActive === boolean) return;
